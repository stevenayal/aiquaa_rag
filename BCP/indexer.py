"""
indexer.py — Pipeline de indexación para el RAG de regulaciones BCP
...
try:
    import pip_system_certs.wrapt_requests  # noqa: F401 — fix SSL Windows
except ImportError:
    pass
...
def upsert_document(sb: Client, record: DocumentRecord) -> Optional[str]:
    data = {
        "source":    getattr(record, "source", "bcp"),
        "url":       record.url,
        "title":     record.title,
...
def index_document(sb: Client,
    ...
    if not text.strip():
        log.warning(f"  → Sin texto (escaneado?): {record.url}")
...
def run_indexer(
    ...
    log.info("\n=== Indexación BCP completa ===")
    log.info(f"  ✓ Exitosos : {ok_count}")
    log.info(f"  ✗ Fallidos : {fail_count}")
    log.info(f"  Total      : {len(records)}")
    if no_embed:
        log.info("  → Embeddings pendientes. Correr: python re_embedder.py --source bcp")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Indexador RAG - BCP")
    parser.add_argument("--categories", nargs="+",
                        choices=["resolucion", "circular", "reglamento", "norma_prudencial"])
    parser.add_argument("--reindex", action="store_true")
    parser.add_argument("--limit", type=int)
    parser.add_argument("--no-embed", action="store_true",
                        help="Guardar texto sin embeddings (no necesita Voyage API key)")
